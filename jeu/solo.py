...
def _cartes_club(jeu, saison: str, team_id: int) -> list[dict]:
    out = []
    for r in jeu.execute("""SELECT c.player_id, j.nom, j.poste, c.ovr, c.attributs, j.postes, j.physique, j.pied, j.pied_faible
                            FROM carte c JOIN joueur j ON j.player_id=c.player_id
        ...
        out.append({"pid": r[0], "nom": r[1], "poste": r[2], "ovr": r[3],
                    ...
                    "pied": r[7] or "droit", "pied_faible": r[8] or 3,
